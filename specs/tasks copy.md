...
**Target Release Date:** 4-Week Build Timeline

**Last Updated:** 2025-12-02

**Related Documentation:**

- PRD: `specs/architecture/prd.md`
- Tech Specs: `specs/architecture/tech-specs.md`
- UI/UX: `specs/architecture/ux.md`
...
- T011 [P]: Define UserProfile type with tier, usage, settings, and Stripe fields (Tech Specs Section 3)
- T012 [P]: Define Note type with content (JSONContent), category, metadata, syncStatus
- T013 [P]: Define Snapshot type for sync conflict recovery
- T014 [P]: Define category enum: pharmacology, med-surg, pediatrics, ob, mental-health, clinical-rotation, other
...
- T018: Define AI types (AutocompleteRequest, AutocompleteResponse, suggestion source)
- T019: Define Stripe types (checkout session, webhook events, subscription status)
- T020: Create shared utility types (Result<T,E>, AsyncState<T>, APIResponse<T>)
- T021: Define error types (AppError, ValidationError, NetworkError, SyncError)
...
- T058: Create local storage abstraction layer (can swap to PowerSync later)
...
- Test save while "offline" (should work locally)
...
- T070: Create template content definitions (Care Plan, Medication Card, SOAP Note, Head-to-Toe, Pathophysiology)
- T071: Create formula content definitions with normal ranges (MAP, BMI, IV Drip, Dosage, GCS, Fluid Deficit)
...
- T097: Create empty state with illustration and "Create Note" CTA (UX Section 4.3)
...
- T112: Create trash empty state with empty bin illustration
...
- T134: Implement Tiptap content to Markdown converter
- T135: Implement Tiptap content to Plain Text converter
...
- T161: Create Supabase project and configure environment variables
...
- [ ] Notes sync to Supabase automatically when online (FR-017)
...
- [ ] Real AI suggestions appear within 200ms of typing (FR-001)
...
- T219: Write unit tests for Zod validation schemas
...
- MS-002 (Types) depends on MS-001 (Environment setup)
- MS-003 (Mocks) depends on MS-002 (Types must exist to mock)
- MS-004 through MS-016 depend on MS-003 (All UI uses mocks)
- MS-017 (Supabase) can start after MS-009 (once core UI works)
- MS-018 (Real Auth) depends on MS-017 (Supabase setup)
- MS-019 (PowerSync) depends on MS-017 (Supabase setup)
- MS-020 (Conflict Resolution) depends on MS-019 (PowerSync)
- MS-021 (Real AI) can start after MS-017 (independent of sync)
- MS-022 (Stripe) depends on MS-018 (Real Auth for user context)
- MS-023 (Testing) depends on all feature milestones
- MS-024 (Performance) depends on MS-023 (Optimize after tests pass)
- MS-025 (Deployment) depends on MS-024 (Deploy optimized build)

**Critical Path:**

T001 → T011 → T024 → T036 → T046 → T067 → T088 → T161 → T178 → T194 → T235

(Setup → Types → Mocks → Auth UI → Editor → Slash Commands → Notes UI → Supabase → PowerSync → AI → Deploy)
...
- MS-001 tasks T001, T002, T003 can run in parallel (different domains)
- MS-002 tasks T011, T012, T013, T014 can run in parallel (independent type definitions)
- MS-003 tasks T024, T025, T026, T027 can run in parallel (independent mock modules)
- MS-004 through MS-008 can have some parallel work (different UI domains)
- MS-017, MS-021 can run in parallel (Supabase setup and AI integration are independent)
...
1. **Week 1 Priority:** Complete MS-001 (Environment), MS-002 (Types), MS-003 (Mocks)
2. **Week 2 Priority:** Complete MS-004 through MS-009 (Core UI with mocks)
3. **Week 3 Priority:** Complete MS-010 through MS-016 (Remaining UI), start MS-017 through MS-019 (Backend)
4. **Week 4 Priority:** Complete MS-020 through MS-025 (Integration, Testing, Deployment)
